...
const UI = (function () {
  ...
  function renderArenaLibrary() {
    ...
    if (!box || typeof Arena === 'undefined') return;
    const games = Arena.games();
    box.innerHTML = games.map(function (g) {
      if (g.status === 'live') {
        return '<button type="button" class="action-card primary text-left" data-screen="' + g.screen + '">' +
          '<div class="action-icon">' + g.emoji + '</div>' +
          '<div class="action-content"><span class="action-title">' + g.title + '</span>' +
          '<span class="action-sub">' + g.blurb + '</span></div>' +
          '<div class="action-badge">Play</div></button>';
      }
      return '<div class="action-card"><div class="action-icon">' + g.emoji + '</div>' +
        '<div class="action-content"><span class="action-title">' + g.title + '</span>' +
        '<span class="action-sub">Coming Soon</span></div></div>';
    }).join('');
  }
