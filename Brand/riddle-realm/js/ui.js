...
const UI = (function () {
  ...
  function bindEvents() {
    ...
    const regBtn = $('app-auth-register');
    if (regBtn) {
      regBtn.addEventListener('click', async () => {
        const msg = $('app-auth-msg');
        if (typeof Auth === 'undefined') {
          if (msg) msg.textContent = 'Auth is not loaded.';
          return;
        }
        if (msg) msg.textContent = 'Creating account…';
        const res = await Auth.register({
          displayName: ($('app-auth-name') || {}).value,
          username: ($('app-auth-user') || {}).value,
          email: ($('app-auth-email') || {}).value,
          password: ($('app-auth-pass') || {}).value,
          role: 'player'
        });
        if (msg) {
          msg.className = res.ok ? 'f-msg ok' : 'f-msg err';
          msg.textContent = res.ok ? 'Account created. You are signed in.' : (res.error || 'Sign up failed');
        }
        const st = $('app-auth-status');
        if (st && res.ok) st.textContent = 'Signed in as ' + ((res.user && res.user.username) || '');
        if (res.ok && typeof Storage !== 'undefined') {
          Storage.resetPlayer();
          Storage.savePlayer({
            name: (res.user && (res.user.displayName || res.user.username)) || 'Player',
            level: 1,
            xp: 0,
            xpNeeded: 100,
            coins: 0
    ...
    const loginApp = $('app-auth-login');
    if (loginApp) {
      loginApp.addEventListener('click', async () => {
        const msg = $('app-auth-msg');
        if (typeof Auth === 'undefined') {
          if (msg) msg.textContent = 'Auth is not loaded.';
          return;
        ...
        if (msg) {
          msg.className = res.ok ? 'f-msg ok' : 'f-msg err';
          msg.textContent = res.ok ? 'Logged in.' : (res.error || 'Login failed');
        }
        const st = $('app-auth-status');
        if (st && res.ok) st.textContent = 'Signed in as ' + ((res.user && res.user.username) || '');
      });
    ...
    if (musicBtn) {
      musicBtn.addEventListener('click', () => {
        ...
        const on = settings.music !== false;
        Storage.saveSettings({ music: !on });
        if (typeof AudioFX !== 'undefined') {
          AudioFX.onMusicSettingChange(!on);
          if (!on) AudioFX.startMusic('spark');
        }
  ...
  function init() {
    ...
    try {
      ...
      if (params.get('fresh') === '1' && typeof Storage !== 'undefined') {
        Storage.resetPlayer();
        Storage.savePlayer({ level: 1, xp: 0, xpNeeded: 100, coins: 0 });
        refreshPlayerUI();
      }
