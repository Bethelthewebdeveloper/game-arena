...
function setSessionCookie(res, sid, secure) {
  const flags = ["ga.sid=" + sid, "HttpOnly", "Path=/", "SameSite=Lax", "Max-Age=" + (60 * 60 * 24 * 14)];
  if (secure) flags.push("Secure");
  res.setHeader("Set-Cookie", flags.join("; "));
}

function clearSessionCookie(res, secure) {
  const flags = ["ga.sid=", "HttpOnly", "Path=/", "SameSite=Lax", "Max-Age=0"];
  if (secure) flags.push("Secure");
  res.setHeader("Set-Cookie", flags.join("; "));
}
...
async function handleRequest(req, res) {
  try {
    ...
    if (method === "POST" && pathname === "/api/register") {
      ...
      setSessionCookie(res, createSession(user.id), isSecureReq(req));
      return sendJson(res, 200, { user: withLevel(user) });
    }
