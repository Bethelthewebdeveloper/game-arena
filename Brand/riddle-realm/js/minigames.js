...
const MiniGames = (function () {
  ...
  function finish(meta, bonusXp, bonusCoins) {
    ...
    box.innerHTML =
      ...
      '<button type="button" class="btn-secondary" id="mg-continue">Continue</button>' +
    ...
    const cont = $('mg-continue');
    if (cont) cont.onclick = function () { renderSetup(); };
  ...
  function playReflex() {
    ...
    function paint(state, msg) {
      const bg = state === 'go' ? '#22c55e' : state === 'wait' ? '#f59e0b' : '#7c5cff';
      play.innerHTML = '<p class="panel-sub">Stage ' + (n + 1) + ' · Round ' + (n + (state === 'go' || state === 'wait' ? 1 : 0)) + ' / ' + goal + '</p>' +
        '<button type="button" id="rf-pad" class="btn-primary" style="min-height:200px;width:100%;font-size:1.6rem;background:' + bg + '">' + msg + '</button>' +
        '<p class="f-note" id="rf-note">Wait for green, then tap.</p>';
      const pad = $('rf-pad');
      pad.onclick = onTap;
  ...
  function playPenalty() {
    ...
    function render(msg) {
      play.innerHTML = '<p class="panel-sub">Shot ' + Math.min(shot + 1, total) + ' / ' + total + ' · Goals ' + goals + '</p>' +
        '<div style="background:#166534;border:4px solid #fff;border-radius:12px;padding:12px;text-align:center;margin-bottom:8px">🥅 GOAL<br/>🧍 keeper</div>' +
        '<p>' + (msg || 'Shoot LEFT · CENTER · RIGHT') + '</p>' +
  ...
  function playRps() {
    ...
    function render(msg) {
      play.innerHTML = '<p class="panel-sub">W ' + wins + ' · L ' + losses + ' · D ' + draws + ' · Streak ' + streak + '</p>' +
        '<p>' + (msg || 'Choose') + '</p>' +
        opts.map(function (o) {
          const icon = o === 'Rock' ? '🪨' : o === 'Paper' ? '📄' : '✂️';
          return '<button type="button" class="btn-primary" data-o="' + o + '">' + icon + ' ' + o + '</button>';
        }).join(' ');
  ...
  function playTrivia() {
    ...
    function nextQ() {
      ...
      const item = pool[n];
      n += 1;
      let leftSec = Math.round(limit / 1000);
      play.innerHTML = '<p class="panel-sub">' + item.cat + ' · Q ' + n + '/' + goal + ' · Streak ' + streak + '</p>' +
        '<h3>' + item.q + '</h3><p id="tr-time" style="font-size:2rem;font-weight:800">' + leftSec + '</p>' +
        item.a.map(function (opt, idx) { return '<button type="button" class="btn-secondary" data-i="' + idx + '">' + opt + '</button>'; }).join('');
      const tick = setInterval(function () {
        leftSec -= 1;
        const t = $('tr-time');
        if (t) t.textContent = String(Math.max(0, leftSec));
        if (leftSec <= 0) { clearInterval(tick); wrong += 1; streak = 0; nextQ(); }
      }, 1000);
